...
def creer_liste(soupe_resultats, mot):
    ...
    for resultat in soupe_resultats:
        ...
        prix_produit = resultat.find('td', {'width': '78'}).text.strip().replace(' / ch.', '')
        ...
        details_produit = {'nom': nom_produit[:40], 'format': format_produit, 'origine': origine_produit, 'prix': str('{}'.format(prix_produit)) + ' $', 'période spécial': periode_special_produit, 'magasin': magasin_produit}
